...
- 現在のリリース版、UpdateページURL、イベント期間、platform別配信状況を公開APIで提供する
...
言語、platform、全8状態を実際の判定処理と翻訳設定で確認できます。
`app-update-landing-server/`で次を実行します。

```bash
composer preview
```

起動後に `http://127.0.0.1:8080/` を開いてください。別のportを使う場合は
リポジトリルートから`scripts/preview-event-update 8081`のように指定します。
プレビューはloopbackアドレスとPHP組み込みサーバーに限定され、`dev/`と
リポジトリルートの`scripts/`は本番デプロイ対象に含まれません。
