...
翻訳結果は元ファイルへそのまま上書きする完全版ではなく、マージ用のJSONとして出力します。共通UI文言では期間中の`button.update`、開始前の`button.prepare`などの文言キー、ゲーム固有文言では`title`、`descriptions`、`socialCard`などの階層を維持し、その配下には依頼されたlocaleだけを含めます。
